...
for i, titulo in enumerate(titulos_finais):
    with tabs[i]:
        if titulo == "📋 Consulta de Escala":
            ...
            df_c = carregar_dados_db().sort_values(by=["data", "hora_entrada"], ascending=[False, False])
            if not df_c.empty:
            ...
            with col_g2:
                ...
                # Ordenação dupla para garantir os registros mais novos e mais recentes no topo
                df_del = carregar_dados_db().sort_values(by=['data', 'hora_entrada'], ascending=[False, False])
                if not df_del.empty:
                    df_del['txt'] = df_del['data'] + " | " + df_del['hora_entrada'] + " | " + df_del['municipio']
            ...
            # --- NOVA SEÇÃO: BASE INTEGRADA ---
            st.divider()
            st.subheader("🗄️ Base Integrada")
            df_base = carregar_dados_db()
            if not df_base.empty:
                # Classificação dupla: Data mais nova > Hora mais nova
                df_base = df_base.sort_values(by=['data', 'hora_entrada'], ascending=[False, False])
                df_base['Situação'] = df_base['cumprido'].map({True: "✅ Cumprida", False: "⚠️ Agendada"})
                
                # Exibe a tabela organizada
                st.dataframe(
                    df_base[['data', 'hora_entrada', 'hora_saida', 'municipio', 'unidade', 'Situação', 'missao', 'criado_por']], 
                    use_container_width=True, 
                    hide_index=True
                )
            else:
                st.info("Nenhum registro encontrado na Base Integrada.")
